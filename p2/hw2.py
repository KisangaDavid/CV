import numpy as np
import cv2 # only used for scaling image for multi-scale object_detection
import math
...
def object_detection(template_images, template_masks, test_img, multi_scale = False):
   ...
   for idx, template_image in enumerate(template_images):
      ...
      for scale in scales:
         ...
         xs0, ys0, scores0 = find_interest_points(cur_template_masked, max_points = max_points)
         feats0 = extract_features(cur_template, xs0, ys0)
